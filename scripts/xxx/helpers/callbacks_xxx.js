...
function addEventListener(event, listener, bRegister = true) { // eslint-disable-line no-redeclare
	if (!Object.hasOwn(callbacks, event)) { console.log('addEventListener：事件不存在 -> ' + event); return false; }
	const id = UUID();
...
function removeEventListeners(event) {
	if (Array.isArray(event)) {
		...
		if (!Object.hasOwn(callbacks, event)) { console.log('removeEventListeners：事件不存在 -> ' + event); return false; }
...
callbacksListener.checkPanelNames = function () {
	if (!window.Name.length) {
		console.popup('面板没有名称："" (' + window.ScriptInfo.Name + ') \n\n请在 SMP 面板配置中更改。', '按钮：检查面板名称');
	} else {
	...
	switch (name) {
		...
		case 'xxx-scripts: panel name reply': {
			if (callbacksListener.listenNames && info === window.Name) {
				console.popup('还有一个面板名称相同："' + info + '"\n\n名称必须不同才能运行动态菜单。请在 SMP 面板配置中更改它。', window.Name);
