...
class Populate {
	...
	butTooltipFont() {
		return ['Microsoft YaHei UI', 14 * $.scale * ppt.zoomTooltipBut / 100, 0];
	}
	...
	calcStatistics(v) {
		...
		switch (ppt.itemShowStatistics) {
			...
			case 5: // popularity
				...
				if (panel.imgView && this.label) value = (ppt.itemShowStatistics == 4 ? '等级 ' : '人气 ') + value;
				this.cache[type][key] = {
					value: value,
					items: items
				}
			...
			case 11: // added
				...
				if (!date) return '';
				if (panel.imgView && this.label) date = ['', '', '', '', '', '', (v.root ? '首次发布 ' : ''), '', '', '首次播放 ', '最近播放 ', '添加时间 '][ppt.itemShowStatistics] + date;
				this.cache[type][key] = {
					value: date,
					items: items
				}
			...
			case 7: {// queue
				...
				if (!index) return '';
				if (panel.imgView && this.label) index = '播放队列 ' + index;
				this.cache[type][key] = {
					value: index,
					items: items
				}
			...
			case 8: {// playcount
				...
				if (panel.imgView) playcount = (this.label ? '播放次数 ' : '')  + playcount + '次';
				this.cache[type][key] = {
					value: playcount,
					items: items
				}
			...
			case 14: // Feedback
				...
				if (panel.imgView && this.label) {
					value = (ppt.itemShowStatistics == 14 ? '反馈 ' : (ppt.itemShowStatistics == 13 ? '不喜欢 ' : '喜爱 ')) + value;
				}
				this.cache[type][key] = {
					value: value,
					items: items
			...
			case !panel.imgView: {
				...
				if (trace2) {
					text = this.statisticsShow
						...
						: (item.count ? ['', '首', '项目'][this.nodeCounts] + ':' + item.count : '');
	...
	draw(gr) {
		...
		if (!this.tree.length || !panel.draw) return gr.GdiDrawText(this.libItems && !panel.search.txt && !ppt.filterBy && ppt.libSource ? '加载中...\n\n' : lib.none, ui.font.main, ui.col.text, ui.sz.margin, panel.search.h, panel.tree.w, ui.row.h * 3);
	...
	lbtn_up(x, y) {
		if (lib.empty && ppt.libSource == 1 && !ppt.fixedPlaylist && y > panel.search.h) { fb.RunMainMenuCommand('Library/Configure'); fb.RunMainMenuCommand('媒体库/配置'); };
		// if (lib.empty && ppt.libSource == 1 && !ppt.fixedPlaylist) fb.RunMainMenuCommand(fb.TitleFormat('$meta()').Eval(true) == '[未知函数]' ? '媒体库/配置' : 'Library/Configure');
		this.last_pressed_coord = {
			x: undefined,
			y: undefined
		};
	...
	load({ handleList, bAddToPls = false, bAutoPlay = false, bUseDefaultPls = false, bInsertToPls = false, bApplySort = true } = {}) {
		...
		if (fb.IsPlaying && !bAddToPls) {
			if (ppt.actionMode == 1) {
				const pl_playing = `${ppt.libPlaylist}（正在播放）`;
	...
	setValues() {
		...
		// Regorxxx <- New statistics
		this.statistics = ['', '比特率', '时长', '合计大小', '等级', '人气', '日期', '播放队列', '播放次数', '首次播放', '最近播放', '添加时间', '喜爱', '不喜欢', '反馈'];
		{
