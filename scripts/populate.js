...
class Populate {
	...
	draw(gr) {
		...
		if (!this.tree.length || !panel.draw) return gr.GdiDrawText(this.libItems && !panel.search.txt && !ppt.filterBy && ppt.libSource ? '加载中...' : lib.none, ui.font.main, ui.col.text, ui.sz.margin, panel.search.h, panel.tree.w, ui.row.h, 0x00000004 | 0x00000400);
	...
	lbtn_up(x, y) {
		if (lib.empty && ppt.libSource == 1 && !ppt.fixedPlaylist) { fb.RunMainMenuCommand('Library/Configure'); fb.RunMainMenuCommand('媒体库/配置'); };
